...
public class Pager {
    ...
    public Pager(int count, int curPage) {
        curBlock = 1;            // 현재블록 번호
        this.curPage = Math.max(curPage, 1);    // 현재 페이지 번호
    ...
    public void setBlockRange() {
        ...
        blockBegin = Math.max((curBlock - 1) * BLOCK_SCALE + 1, 1);
        blockEnd = Math.min(blockBegin + BLOCK_SCALE - 1, totPage);

        // [이전], [다음]을 눌렀을 때 이동할 페이지 번호 계산
        prevPage = Math.max((curBlock == 1) ? 1 : (curBlock - 1) * BLOCK_SCALE, 1);
        nextPage = Math.min(curBlock < totBlock ? (curBlock * BLOCK_SCALE) + 1 : totPage, totPage);
    }
    ...
    public void setCurPage(int curPage) {
        this.curPage = Math.max(curPage, 1);
    }
    ...
    public void setTotPage(int count) {
        // Math.ceil() 올림
        totPage = Math.max((int) Math.ceil(count * 1.0 / PAGE_SCALE), 1);
    }
